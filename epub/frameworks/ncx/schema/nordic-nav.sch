...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <let name="NCX_DOC"
        value="document(replace(base-uri(), '/[^/]+$', '/nav.ncx'))"/>
    <let name="NCX_NAV" value="$NCX_DOC/ncx:ncx/ncx:navMap"/>
    <let name="NCX_PAGES" value="$NCX_DOC/ncx:ncx/ncx:pageList"/>
    <let name="XHTML_NAV_DOC"
        value="document(replace(base-uri(), '/[^/]+$', '/nav.xhtml'))"/>
    <let name="XHTML_NAV"
        value="$XHTML_NAV_DOC/html:html/html:body/html:nav[tokenize(@epub:type, '\s+') = 'toc']"/>
    <let name="XHTML_PAGES"
        value="$XHTML_NAV_DOC/html:html/html:body/html:nav[tokenize(@epub:type, '\s+') = 'page-list']"/>
    <!-- navdoc toc items must exist in the ncx -->
    <pattern>
        <rule context="html:a[ancestor::html:nav/tokenize(@epub:type,'\s+')='toc']">
            <let name="navPoint" value="$NCX_NAV//ncx:navPoint[ncx:content/@src=current()/@href]"/>
    ...
    <pattern>
        <rule context="html:a[ancestor::html:nav/tokenize(@epub:type,'\s+')='page-list']">
            <let name="pageTarget" value="$NCX_PAGES/ncx:pageTarget[ncx:content/@src=current()/@href]"/>
    ...
    <pattern>
        <rule context="ncx:navPoint">
            <let name="navRef" value="$XHTML_NAV//html:a[@href=current()/ncx:content/@src]"/>
    ...
    <pattern>
        <rule context="ncx:pageTarget">
            <let name="navRef" value="$XHTML_PAGES//html:a[@href=current()/ncx:content/@src]"/>
    ...
    <pattern>
        <rule context="ncx:navLabel[parent::ncx:navMap]">
            <assert test="normalize-space(string-join(.//text(),'')) = /$XHTML_NAV/html:h1/normalize-space(string-join(.//text(),''))"
    ...
    <pattern>
        <rule context="ncx:navLabel[parent::ncx:pageList]">
            <assert test="normalize-space(string-join(.//text(),'')) = /$XHTML_PAGES/html:h1/normalize-space(string-join(.//text(),''))"
